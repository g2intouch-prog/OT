 ...
 */
async function createUser(username, password, publicKey = null, encryptedPrivateKey = null, salt = null) {
  ...
  if (process.env.POSTGRES_URL) {
    try {
      await db.query(
        ...
        [userId, username.trim(), password, 'user', 'active', publicKey, encryptedPrivateKey, salt]
      );
      return { id: userId, username, role: 'user', status: 'active', totp_enabled: 0, public_key: publicKey, encrypted_private_key: encryptedPrivateKey, salt };
    } catch (e) {
