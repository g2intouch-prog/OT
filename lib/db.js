...
const inMemoryDb = {
  users: [
    { id: 'usr-admin-master', username: 'g2intouch@gmail.com', role: 'admin', status: 'active', password: 'password', totp_enabled: 0, totp_secret: null }
  ],
  sessions: {},
  records: []
};
...
async function initDb() {
  ...
  try {
    ...
    // Clean up legacy demo accounts from Postgres database table if present
    await db.query("DELETE FROM users WHERE LOWER(username) IN ('admin@vault.team', 'user@vault.team', 'revoked@vault.team', 'admin', 'user')");

    // Seed default master admin (g2intouch@gmail.com) if no users exist
    const masterAdminCheck = await db.query("SELECT * FROM users WHERE LOWER(username) = 'g2intouch@gmail.com'");
    if (masterAdminCheck.rowCount === 0) {
      await db.query(
        "INSERT INTO users (id, username, password, role, status, totp_enabled) VALUES ($1, $2, $3, $4, $5, 0) ON CONFLICT (username) DO NOTHING",
        ['usr-admin-master', 'g2intouch@gmail.com', 'password', 'admin', 'active']
      );
    }
...
async function verifyUserSession(req) {
  ...
  if (process.env.POSTGRES_URL) {
    const { rows } = await db.query(
      `SELECT s.user_id, s.token, s.expires_at, u.username, u.role, u.status, u.public_key, u.encrypted_private_key, u.wrapped_vault_key, u.salt
       FROM sessions s
    ...
    const user = inMemoryDb.users.find(u => u.id === session.user_id);
    if (!user) return null;
    return { user_id: user.id, token, expires_at: session.expires_at, username: user.username, role: user.role, status: user.status, public_key: user.public_key, encrypted_private_key: user.encrypted_private_key, wrapped_vault_key: user.wrapped_vault_key, salt: user.salt };
  }
}

/**
 * Checks role and status of user.
 */
async function checkUserStatusAndRole(userId) {
  if (process.env.POSTGRES_URL) {
    const { rows } = await db.query("SELECT id, role, status FROM users WHERE id = $1", [userId]);
    return rows.length > 0 ? rows[0] : null;
  } else {
...
/**
 * Saves entry in PostgreSQL database
 */
async function saveEncryptedEntry(userId, ciphertext, iv) {
  if (process.env.POSTGRES_URL) {
    const res = await db.query(
      "INSERT INTO encrypted_records (user_id, ciphertext, iv) VALUES ($1, $2, $3) RETURNING id, user_id, ciphertext, iv, created_at",
...
async function getAllUsers() {
  if (process.env.POSTGRES_URL) {
    const { rows } = await db.query(`
      ...
      FROM users u
      WHERE LOWER(u.username) NOT IN ('admin@vault.team', 'user@vault.team', 'revoked@vault.team', 'admin', 'user')
    `);
    return rows;
  } else {
    const now = Date.now();
    ...
    return inMemoryDb.users
      .filter(u => !['admin@vault.team', 'user@vault.team', 'revoked@vault.team', 'admin', 'user'].includes(u.username.toLowerCase()))
      .map(u => ({
        id: u.id,
        username: u.username,
        role: u.role,
...
/**
 * Deletes all user accounts except the active admin account.
 */
async function clearAllUsersExceptMaster(adminUserId) {
  ...
  if (process.env.POSTGRES_URL) {
    // Delete all sessions except the active admin
    await db.query("DELETE FROM sessions WHERE user_id <> $1", [adminUserId]);
    // Delete all users except the active admin
    const res = await db.query("DELETE FROM users WHERE id <> $1", [adminUserId]);
    return res.rowCount;
  } else {
    const originalCount = inMemoryDb.users.length;
    inMemoryDb.users = inMemoryDb.users.filter(u => u.id === adminUserId);
...
/**
 * Updates a user's username/email address.
 */
async function updateUserUsername(userId, newUsername) {
  const cleanEmail = (newUsername || '').trim().toLowerCase();
  if (!cleanEmail) throw new Error('Username/Email cannot be empty.');

  if (process.env.POSTGRES_URL) {
    const userCheck = await db.query("SELECT id FROM users WHERE LOWER(username) = $1 AND id <> $2", [cleanEmail, userId]);
    if (userCheck.rowCount > 0) {
      throw new Error('An account with this email/username already exists.');
    }
    await db.query("UPDATE users SET username = $1 WHERE id = $2", [cleanEmail, userId]);
  } else {
    const existing = inMemoryDb.users.find(u => u.username.toLowerCase() === cleanEmail && u.id !== userId);
    if (existing) {
      throw new Error('An account with this email/username already exists.');
    }
    const user = inMemoryDb.users.find(u => u.id === userId);
    if (user) user.username = cleanEmail;
  }
}
...
async function createPasswordResetRequest(username) {
  const cleanUsername = (username || '').trim().toLowerCase();
  if (!cleanUsername) throw new Error('Username/Email is required.');

  // Check if user exists
  let userExists = false;
  if (process.env.POSTGRES_URL) {
    const res = await db.query("SELECT id FROM users WHERE LOWER(username) = $1", [cleanUsername]);
    userExists = res.rowCount > 0;
  } else {
    userExists = inMemoryDb.users.some(u => u.username.toLowerCase() === cleanUsername);
  }
  ...
  if (process.env.POSTGRES_URL) {
    await db.query("INSERT INTO password_reset_requests (username, status) VALUES ($1, 'pending')", [cleanUsername]);
  } else {
    if (!inMemoryDb.password_reset_requests) inMemoryDb.password_reset_requests = [];
    inMemoryDb.password_reset_requests.push({
      id: Date.now(),
      username: cleanUsername,
      status: 'pending',
      created_at: new Date()
 ...
 */
async function getPendingPasswordResetRequests() {
  if (process.env.POSTGRES_URL) {
    const { rows } = await db.query("SELECT * FROM password_reset_requests WHERE status = 'pending' ORDER BY created_at DESC");
    return rows;
  } else {
    if (!inMemoryDb.password_reset_requests) inMemoryDb.password_reset_requests = [];
    return inMemoryDb.password_reset_requests.filter(r => r.status === 'pending');
  }
}

/**
 * Approves a password reset request and resets user's password to a temp password.
 */
async function approvePasswordResetRequest(requestId, tempPassword) {
  let reqItem = null;
  if (process.env.POSTGRES_URL) {
    const { rows } = await db.query("SELECT * FROM password_reset_requests WHERE id = $1", [requestId]);
    if (rows.length > 0) reqItem = rows[0];
  } else {
    if (!inMemoryDb.password_reset_requests) inMemoryDb.password_reset_requests = [];
    reqItem = inMemoryDb.password_reset_requests.find(r => r.id == requestId);
  }

  if (!reqItem) throw new Error('Password reset request not found.');

  const username = reqItem.username.toLowerCase();

  // Reset user password
  if (process.env.POSTGRES_URL) {
    await db.query("UPDATE users SET password = $1 WHERE LOWER(username) = $2", [tempPassword, username]);
    await db.query("UPDATE password_reset_requests SET status = 'approved', temp_password = $1 WHERE id = $2", [tempPassword, requestId]);
  } else {
    const user = inMemoryDb.users.find(u => u.username.toLowerCase() === username);
    if (user) user.password = tempPassword;
    reqItem.status = 'approved';
    reqItem.temp_password = tempPassword;
  }
...
async function rejectPasswordResetRequest(requestId) {
  if (process.env.POSTGRES_URL) {
    await db.query("UPDATE password_reset_requests SET status = 'rejected' WHERE id = $1", [requestId]);
  } else {
    if (!inMemoryDb.password_reset_requests) inMemoryDb.password_reset_requests = [];
    const reqItem = inMemoryDb.password_reset_requests.find(r => r.id == requestId);
