  ...
  if (process.env.POSTGRES_URL) {
    try {
      await db.query(
        ...
        [userId, username.trim(), password, validRole, 'active', publicKey, encryptedPrivateKey, salt]
      );
      return { id: userId, username: username.trim(), role: validRole, status: 'active', totp_enabled: 0, public_key: publicKey, encrypted_private_key: encryptedPrivateKey, salt };
    } catch (e) {
