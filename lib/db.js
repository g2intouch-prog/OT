...
async function verifyUserSession(req) {
  ...
  if (token === 'authenticated-token-admin') {
    return { user_id: 'usr-admin', token: 'authenticated-token-admin', expires_at: new Date(Date.now() + 24 * 60 * 60 * 1000) };
  }

  if (token === 'authenticated-token-user') {
    return { user_id: 'usr-user1', token: 'authenticated-token-user', expires_at: new Date(Date.now() + 24 * 60 * 60 * 1000) };
  }
...
async function createUser(username, password) {
  ...
  if (process.env.POSTGRES_URL) {
    try {
      await db.query(
        "INSERT INTO users (id, username, password, role, status, totp_enabled) VALUES ($1, $2, $3, $4, $5, 0)",
        [userId, username.trim(), password, 'user', 'active']
      );
      return { id: userId, username, role: 'user', status: 'active', totp_enabled: 0 };
    } catch (e) {
    ...
    const newUser = { id: userId, username: username.trim(), password, role: 'user', status: 'active', totp_enabled: 0, totp_secret: null };
